  ...
  return (
    <section id="services" className="relative min-h-screen flex items-center" style={{ background: 'linear-gradient(to bottom right, rgba(254, 172, 214, 0.3) 0%, black 30%, black 70%, rgba(254, 172, 214, 0.3) 100%)' }}>
      <div className="max-w-4xl mx-auto px-6 py-32">
        <h2 className={`font-bold text-white mb-4 md:mb-6 uppercase text-4xl sm:text-6xl md:text-7xl lg:text-6xl xl:text-7xl scroll-animate-up ${isTitleVisible ? 'animate' : ''}`} ref={titleRef} style={{ letterSpacing: '0.15em', fontFamily: 'var(--font-montserrat)' }}>
          {t.services.passion}
        </h2>
        <p className={`font-normal text-white mb-8 md:mb-10 text-lg sm:text-2xl md:text-3xl lg:text-2xl xl:text-3xl leading-relaxed scroll-animate-up ${isSubtitleVisible ? 'animate' : ''}`} ref={subtitleRef} style={{ letterSpacing: '0.2em', fontFamily: 'var(--font-montserrat)' }}>
